...
function WorkspacePage({ selectedPlan, onSelectPlan }) {
  ...
  const updateKeyword = async (accountId, newKeyword) => {
    try {
      const response = await axios.post(`${API_URL}/keyword/${accountId}`, {
        keyword: newKeyword,
      });
      setAccounts(
        accounts.map((account) =>
  ...
  const updatePrompt = async (accountId, newPrompt) => {
    try {
      const response = await axios.post(`${API_URL}/prompt/${accountId}`, {
        prompt: newPrompt,
      });
      setAccounts(
        accounts.map((account) =>
  ...
  const updateKpi = async (accountId, newKpi) => {
    try {
      const response = await axios.post(`${API_URL}/kpi/${accountId}`, {
        kpi: newKpi,
      });
      setAccounts(
        accounts.map((account) =>
          account.id === accountId
            ? { ...account, kpi: response.data.kpi }
            : account
        )
      );
      onKpiModalClose();
